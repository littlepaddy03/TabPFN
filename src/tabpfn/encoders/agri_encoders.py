...
class AgriDataEncoder(InputEncoder):
    ...
    def forward(self, x: torch.Tensor, single_eval_pos: Optional[int] = None) -> torch.Tensor:
        ...
        if self.handle_missing_values == "mean":
            ...
            if temporal_mask.any():
                temporal_data[temporal_mask] = np.take(
                    ...
                    np.nonzero(temporal_mask.reshape(temporal_mask.shape[0], -1))[-1] % self.temporal_means.size
                )
