...
def create_synthetic_agri_data(
    ...
    for i in range(n_samples):
        for j in range(n_features):
            ...
            season = np.sin(np.linspace(0, 4 * np.pi, n_time_steps)) + np.random.normal(0, 0.2, n_time_steps)
    ...
    critical_period_avg = np.nanmean(
        X[:, critical_period_start:critical_period_end, :], 
        axis=1
    )
...
def test_agri_data_encoder():
    ...
    print("\nTesting preprocessing and encoder setup...")
    ...
    print("\nTesting batch encoding for TabPFN...")
    encoded_batch = encode_agri_batch_for_tabpfn(X_processed, encoder)
